...
public class TransactionTest {
    ...
    public void testUpdateBalance() {
        ClassPathXmlApplicationContext applicationContext = new ClassPathXmlApplicationContext("jdbc/transaction.xml");
        UserService userService = (UserService) applicationContext.getBean("userService");
        userService.transfer(3, 2, 1000);
    }

    /**
     * 在用REQUIRES_NEW的时候，发现没有起作用。分析了一下，原因是 A方法（有事务）调用B方法（要独立新事务），如果两个方法写在同一个类里，
     * spring的事务会只处理能同一个。
     */
    @Test
    @Transactional
